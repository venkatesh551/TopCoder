...
def do_test(a, b, c, __expected):
    startTime = time.time()
    instance = PolynomialRemainder()
    exception = None
    try:
        __result = instance.findRoot(a, b, c);
    except:
        import traceback
...
def run_tests():
    sys.stdout.write("PolynomialRemainder (1000 Points)\n\n")
    ...
    with open("PolynomialRemainder.sample", "r") as f:
        while True:
            label = f.readline()
